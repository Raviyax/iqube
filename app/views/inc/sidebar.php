<div id="mySidebar" class="sidebar">
            <div class="sidebar-top" id="sidebar_top">
                <div class="usericon" id="usericon" href="#home"><img src="<?php echo URLROOT;?>/assets/img/Landing/user.jpg" alt=""></div>
                <div class="userdetails" id="userdetails" href="#home">
                    <ul>
                        <li>User</li>
                        <li style="font-size: 10px; opacity: 0.5;">Login</li>
                    </ul>
                </div>
            </div>

            <div class="sidebar_content">
                <a class="active" href="abc.html"><span class="naviconBx"><i class="fa fa-tachometer"
                            aria-hidden="true"></i> </span><span class="naviconBx">About Us</span></a>
               
            </div>
        </div>